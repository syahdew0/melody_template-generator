  ...
  <div class="p-6 space-y-6">
    ...
    <div v-else>
      ...
      <table v-else class="w-full table-auto border border-gray-300 text-sm">
        ...
        <tbody>
          <tr
            v-for="wd in withdraws"
            :key="wd.id"
            :class="{ 'bg-yellow-50': selectedWithdrawIds.has(wd.id) }"
          >
      ...
      <div class="mt-6 text-right space-y-1 text-sm max-w-full mx-auto">
...
const fetchWithdraws = async () => {
  ...
  try {
    ...
    // contoh endpoint: GET /withdraws?status=&username=&fromDate=&toDate=&page=
    const response = await axios.get(API_ENDPOINTS.withdraw.list(), { params })
...
const allSelected = computed(() => {
  const pendingIds = withdraws.value
...
onMounted(() => {
  ...
  // Jangan fetch otomatis, tunggu klik tombol Cari
})
</script>
