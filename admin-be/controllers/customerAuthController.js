...
exports.register = async (req, res) => {
  ...
  try {
    const { username, email, no_hp, bank, no_rekening, nama_rekening, password } = req.body;

    const existingUser = await Customer.findOne({ where: { [Op.or]: [{ username }, { email }] } });
    if (existingUser) return res.status(400).json({ message: 'Username atau email sudah terdaftar.' });

    const hashedPassword = await bcrypt.hash(password, 10);

    const newUser = await Customer.create({
      username, email, no_hp, bank, no_rekening, nama_rekening, password: hashedPassword
    }, { transaction: t });
