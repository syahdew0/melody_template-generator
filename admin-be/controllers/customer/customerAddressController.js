...
// --- Load provinsi data ---
const provincesData = loadJSON(path.join(__dirname, '../../data/indonesia/provinsi.json'));
...
module.exports = {
  ...
  async provinces(req, res) {
    try {
      const provs = provincesData.map(p => ({ id: p.id, name: p.nama }));
      res.json(provs);
    } catch (err) {
  ...
  async regencies(req, res) {
    ...
    if (!province_id) return res.status(400).json({ message: 'province_id wajib' });

    const filePath = path.join(__dirname, `../../data/indonesia/kabupaten/${province_id}.json`);
    if (!fs.existsSync(filePath)) return res.json([]);

    const kabupaten = loadJSON(filePath);
  ...
  async districts(req, res) {
    ...
    if (!regency_id) return res.status(400).json({ message: 'regency_id wajib' });

    const filePath = path.join(__dirname, `../../data/indonesia/kecamatan/${regency_id}.json`);
    if (!fs.existsSync(filePath)) return res.json([]);

    const kecamatan = loadJSON(filePath);
  ...
  async villages(req, res) {
    ...
    if (!district_id) return res.status(400).json({ message: 'district_id wajib' });

    const filePath = path.join(__dirname, `../../data/indonesia/kelurahan/${district_id}.json`);
    if (!fs.existsSync(filePath)) return res.json([]);

    const kelurahan = loadJSON(filePath);
